...
class Users extends CI_Model
{
	...
	{
		$this->db->where('id', $user_id);
		$res = $this->db->update($this->table_name, array(
			'banned'		=> 0,
			'ban_reason'	=> NULL,
		));
		if ($res) {
	...
	function update_profile($user_id, $profile)
	{
		...
		$this->db->update($this->profile_table_name, $profile);
	 ...
	 *
	 */
	function create_company($user_id, $data){
		$values['user_id']     = $user_id;
		$values['name']        = $data['company'];
		$values['country']     = $data['country'];
		$values['phone']       = $data['phone'];

		$query = $this->db->insert(COMPANY, $values); 
		return $query;
	}
	/**
	 *
	 */
	function admin_create_company($data){
